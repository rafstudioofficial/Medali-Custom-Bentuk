...
/* ================================================================
   ...
   CATATAN PERFORMA (kenapa submit/edit kadang terasa lama):
   Setiap panggilan ke Apps Script (baik GET maupun POST) dieksekusi di
   server Google yang sifatnya "on-demand" — bukan server yang selalu
   menyala. Kalau web app ini belum dipakai beberapa saat, request
   PERTAMA setelah itu wajar butuh beberapa detik ekstra untuk "start up"
   (cold start) sebelum kode-nya benar-benar jalan. Ini di luar kendali
   kode di file ini, dan sifatnya memang kadang-kadang (tidak selalu),
   sesuai laporan yang dialami.
   Yang BISA dikendalikan dari kode adalah jumlah request yang dikirim:
   sebelumnya, tiap create/update/delete di sini diikuti oleh 1 request
   GET tambahan dari frontend untuk memuat ulang SEMUA data (baca ulang
   seluruh baris sheet + gabungkan Order Tambahan) — padahal hasil aksi
   itu sendiri sudah cukup untuk update tampilan. Sekarang createData_/
   updateData_/createDataTambahan_/updateDataTambahan_ mengembalikan
   `item` hasil aksinya, dan frontend (lihat sendToGAS() & 
   sendTambahanAction() di script.js) memakai `item` itu langsung tanpa
   perlu GET ulang — jadi tiap submit sekarang cuma 1 request ke Apps
   Script, bukan 2. Ini tidak menghilangkan cold start, tapi memangkas
   separuh kemungkinan kena cold start di tiap submit.
   ================================================================ */
function doPost(e) {
  try {
...
function createData_(sheet, payload) {
  ...
  // `item` dikembalikan supaya frontend bisa langsung update tampilan dari
  // hasil ini TANPA perlu GET ulang seluruh data (lihat catatan performa di
  // atas doPost()) — itu yang sebelumnya bikin submit terasa lambat 2x lipat.
  return { success: true, message: 'Data berhasil ditambahkan', id: id, item: rowToObject_(newRow) };
}
...
function updateData_(sheet, payload) {
  ...
  // Sama seperti createData_ — kembalikan `item` hasil update supaya frontend
  // tidak perlu GET ulang seluruh data.
  return { success: true, message: 'Data berhasil diperbarui', item: rowToObject_(merged) };
}
...
function createDataTambahan_(payload) {
  ...
  sheet.appendRow(newRow);
  return { success: true, message: 'Order tambahan berhasil ditambahkan', id: id, item: rowToObjectTambahan_(newRow) };
}
...
function updateDataTambahan_(payload) {
  ...
  return { success: true, message: 'Order tambahan berhasil diperbarui', item: rowToObjectTambahan_(merged) };
}
