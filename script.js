...
// status: '' (semua) | 'Proses' | 'Selesai'. Default "Proses" supaya Dashboard
// hanya menampilkan medali yang masih dikerjakan; yang sudah selesai disembunyikan
// kecuali user sengaja pilih "Semua Status" atau "Sudah Selesai".
let dashFilters = {
  mode: "",
  bulan: "",
  tanggal: "",
...
function bindFilterGroup(prefix, state, onChange) {
  ...
  // Filter status (Proses/Selesai) — opsional, saat ini cuma ada di Dashboard.
  ...
  resetBtn.addEventListener("click", () => {
    ...
    if (statusSelect) {
      // Reset filter status balik ke default "Sedang Proses", bukan "Semua Status" —
      // supaya perilaku Dashboard tetap konsisten (hanya tampil yang belum selesai).
      state.status = "Proses";
...
function applyFilters(data, filters) {
  return data.filter((item) => {
    ...
    // filters.status cuma ada di grup filter Dashboard (lihat dashFilters).
    // "" berarti Semua Status, jadi tidak difilter.
    ...
    <div class="col-6 col-md-4 col-lg-3 mb-4">
      <div class="card medal-card" style="cursor:pointer;" onclick="showDetailModal('${item.id}')">
        <div class="medal-photo-wrap">
          ${
            item.foto
              ? `<img src="${escapeHtml(fotoUrl(item.foto, 400))}" alt="${escapeHtml(item.produk)}" loading="lazy" decoding="async">`
...
const MAX_FILE_MASTER_DIRECT_BASE64 = 50 * 1024 * 1024; // 50MB
  ...
  fetch(GAS_URL, {
    ...
    .then((json) => {
      if (json.success) {
        ...
        resetForm();
        applyMutationResult_(action, payload, json);
        renderDashboard();
        if (isAuthenticated()) {
          renderInputTable();
          renderFileMaster();
...
/**
 * Perbarui cache lokal (medaliData) berdasarkan hasil create/update/delete
 * dari server, TANPA fetch ulang seluruh data (GET) ke Apps Script.
 *
 * KENAPA INI PENTING UNTUK PERFORMA:
 * Sebelumnya, tiap submit/edit/hapus selalu diikuti 1 request GET tambahan
 * (loadDataFromServer) yang membaca ulang SEMUA baris sheet + menggabungkan
 * Order Tambahan dari awal. Jadi tiap aksi = 2 request ke Apps Script
 * (POST lalu GET), dan tiap request punya risiko kena "cold start" Apps
 * Script (lihat catatan di Code.gs doPost()) — makanya submit sering
 * terasa dobel lama. Sekarang backend (Code.gs) sudah dimodifikasi untuk
 * mengembalikan `item` hasil aksinya di response POST, jadi cukup dipakai
 * langsung untuk update tampilan di sini — 1 aksi = 1 request saja.
 */
function applyMutationResult_(action, payload, json) {
  if (action === "create" && json.item) {
    const item = json.item;
    item.orderTambahan = [];
    item.total = item.jumlah || 0;
    medaliData.push(item);
  } else if (action === "update" && json.item) {
    const idx = medaliData.findIndex(
      (d) => String(d.id) === String(payload.id),
    );
    if (idx !== -1) {
      // Order tambahan tidak ikut dikirim/diubah oleh aksi update ini,
      // jadi dipertahankan dari data lama yang sudah ada di cache.
      const oldTambahan = medaliData[idx].orderTambahan || [];
      const merged = Object.assign({}, json.item, {
        orderTambahan: oldTambahan,
      });
      merged.total =
        (merged.jumlah || 0) +
        oldTambahan.reduce((sum, t) => sum + (t.jumlah || 0), 0);
      medaliData[idx] = merged;
    } else {
      // Jarang terjadi (mis. data dihapus user lain barusan) -> sync ulang.
      loadDataFromServer(true);
    }
  } else if (action === "delete") {
    medaliData = medaliData.filter((d) => String(d.id) !== String(payload.id));
  } else {
    // Jaga-jaga kalau suatu saat action baru ditambahkan tapi lupa
    // ditangani di sini -> tetap fallback ke reload penuh biar aman.
    loadDataFromServer(true);
  }
}
...
function renderInputTable() {
  ...
  tbody.innerHTML = filtered
    .map((item) => {
      ...
      return `
    <tr>
      <td>${item.foto ? `<img src="${escapeHtml(fotoUrl(item.foto, 150))}" class="table-thumb" loading="lazy" decoding="async">` : '<div class="table-thumb"></div>'}</td>
...
function showDetailModal(id) {
  ...
  // Tombol "Tandai Selesai" — sengaja TIDAK dibatasi login (bisa dipakai
  // siapa pun yang buka Dashboard, sesuai permintaan). Cuma disembunyikan
  // kalau status saat ini sudah "Selesai" (buat apa lagi ditandai ulang).
  const markBtn = document.getElementById("markSelesaiBtn");
  markBtn.dataset.medaliId = item.id;
...
/**
 * Tombol cepat "Tandai Selesai" di modal Detail Medali (Dashboard).
 * Supaya kalau order sudah kelar sebelum 7 hari (batas otomatis), operator
 * tidak perlu buka menu Input Data dulu buat ganti status — cukup dari
 * kartu di Dashboard, klik, langsung tersimpan ke server.
 */
function bindMarkSelesaiButton() {
  document.getElementById("markSelesaiBtn").addEventListener("click", (e) => {
    const id = e.currentTarget.dataset.medaliId;
    if (!id) return;
    if (!confirm("Tandai medali ini sebagai Selesai?")) return;

    sendToGAS(
      "update",
      { id: id, status: "Selesai" },
      "Status berhasil diubah menjadi Selesai",
    );

    // Sinkronkan tampilan modal yang sedang terbuka tanpa perlu ditutup dulu.
    document.getElementById("detailStatus").innerHTML = statusBadge("Selesai");
  ...
  fetch(GAS_URL, {
    ...
    .then((json) => {
      if (json.success) {
        ...
        applyTambahanMutationResult_(action, payload, json);
        resetTambahanForm();
        const item = medaliData.find(
...
/**
 * Sama seperti applyMutationResult_(), tapi untuk aksi Order Tambahan —
 * update array `orderTambahan` di dalam produk induknya (medaliData) secara
 * lokal, tanpa GET ulang seluruh data. Lihat catatan performa di
 * applyMutationResult_() untuk alasannya.
 */
function applyTambahanMutationResult_(action, payload, json) {
  const parent = medaliData.find(
    (d) => String(d.id) === String(tambahanParentId),
  );
  if (!parent) {
    loadDataFromServer(true); // jaga-jaga, seharusnya tidak terjadi
    return;
  }
  if (!parent.orderTambahan) parent.orderTambahan = [];

  if (action === "createTambahan" && json.item) {
    parent.orderTambahan.push(json.item);
  } else if (action === "updateTambahan" && json.item) {
    const idx = parent.orderTambahan.findIndex(
      (t) => String(t.id) === String(payload.id),
    );
    if (idx !== -1) parent.orderTambahan[idx] = json.item;
    else parent.orderTambahan.push(json.item);
  } else if (action === "deleteTambahan") {
    parent.orderTambahan = parent.orderTambahan.filter(
      (t) => String(t.id) !== String(payload.id),
    );
  }
  ...
  parent.total =
    (parent.jumlah || 0) +
    parent.orderTambahan.reduce((sum, t) => sum + (t.jumlah || 0), 0);
}
