  ...
  if (file.size > MAX_FILE_MASTER_UPLOAD_SIZE) {
    showToast(
      "File terlalu besar (" +
        ...
        "MB). Maksimal " + (MAX_FILE_MASTER_UPLOAD_SIZE / (1024 * 1024)) + "MB.",
      "danger",
    );
    e.target.value = "";
    return;
...
function uploadFileMasterDirect(file) {
  ...
  fetch(GAS_URL, {
    ...
    .then((res) => res.json())
    .then((json) => {
      if (!json.success) throw new Error(json.message || "Gagal membuat sesi upload");
      progressBar.textContent = "0%";
      return putFileToUploadUrl_(json.uploadUrl, file, progressBar);
    })
    .then((driveFile) => {
      fileMasterDirectUrl = "https://drive.google.com/uc?export=download&id=" + driveFile.id;
...
function handleFormSubmit(e) {
  ...
  } else if (fileMasterUrlTyped && fileMasterUrlTyped !== fileMasterExisting.url) {
...
function loadItemToForm(id) {
  ...
  fileMasterNamaBaru = "";
  removeFileMaster = false;
  fileMasterExisting = { url: item.fileMaster || "", nama: item.fileMasterNama || "" };
  document.getElementById("inputFileMaster").value = "";
  document.getElementById("inputFileMasterUrl").value = "";
  document.getElementById("fileMasterProgressWrap").classList.add("d-none");
  renderFileMasterInfo();
...
function bindDetailPdfButton() {
  document
    ...
    .addEventListener("click", () => {
      const id = document.getElementById("downloadDetailPdfBtn").dataset.medaliId;
  ...
  window.addEventListener("unhandledrejection", (e) => {
    ...
    showLoading(false);
    showToast(
      "Terjadi kesalahan tak terduga: " + (e.reason && e.reason.message ? e.reason.message : e.reason),
      "danger",
    );
  });
}
...
async function downloadDetailPdf(id) {
  ...
  showLoading(true);
  try {
    const photoData = item.foto ? await imageUrlToDataUrl(fotoUrl(item.foto, 1000)) : null;
...
async function buildDetailPdf(item, photoData) {
  ...
  doc.text("Dicetak: " + generatedAt, pageWidth - marginX, 20, { align: "right" });
  ...
  for (let i = 1; i <= pageCount; i++) {
    ...
    doc.text(`Halaman ${i} / ${pageCount}`, pageWidth - marginX, pageHeight - 9, {
      align: "right",
    });
  }

  const filenameParts = [item.mitra, item.produk, item.namaMedali].filter(Boolean);
